...
def test(model_directory, batch_size):
    ...
    y = cnn.CNN(x, is_training=is_training)
    ...
    for i in range(total_batch):
        ...
        tf.summary.scalar('acc', numpy.mean(acc_buffer))

        # Merge all summaries into a single op
        merged_summary_op = tf.summary.merge_all()

        #TODO: error in showing test in tensorboard

        y_final, summary = sess.run([y, tf.summary.merge_all()], feed_dict={x: batch_xs, y_: batch_ys, is_training: False})
...
"""Result: 
##### 15.174624919891357 seconds #####
test accuracy for the stored model: 0.9926
"""
